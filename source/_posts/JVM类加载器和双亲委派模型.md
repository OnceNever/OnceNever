---
title: JVM类加载器和双亲委派模型
date: 2024/01/06 13:00:18
categories:
- [JVM, 类加载器]
tags:
- JVM
...
在了解类加载器之前首先我们需要知道在`JVM`中类加载的过程步骤为：**加载 -> 连接 -> 初始化**。

而连接过程又可以进一步细分为三步：**验证 ->  准备 -> 解析**。

![JVM类加载过程](https://image.seeyourface.cn/2024/01/e756e4b0cdae0889ccd514ce59cfefac.png)
...
总而言之，**类加载器的主要作用就是加载 Java 类的字节码（ `.class` 文件）到 JVM 中（在内存中生成一个代表该类的 `Class` 对象）。** 字节码可以是 Java 源程序（`.java`文件）经过 `javac` 编译得来，也可以是通过工具动态生成或者通过网络下载得来。
...
对于已经加载的类会被放在 `ClassLoader` 中。在类加载的时候，系统会首先判断当前类是否被加载过。已经被加载的类会直接返回，否则才会尝试加载。也就是说，对于**同一个类加载器来说**，相同二进制名称的类只会被加载一次。

在`JVM`中，一个类的唯一性是需要这个类本身和类加载一起才能确定的，每个类加载器都有一个独立的命名空间。

不同的类加载器，即使是同一个类字节码文件，最后在`JVM`里的类对象也不是同一个，我们可以通过下面这段代码来验证这个结论：
...
public class ClassLoaderTest {
    public static void main(String[] args) throws ClassNotFoundException, IllegalAccessException,
        ...
        // 自定义类加载器加载
        Object myClassLoaderTest = myLoader.loadClass("cn.seeyourface.ClassLoaderTest").newInstance();
        System.out.println(myClassLoaderTest.getClass());
        System.out.println(myClassLoaderTest instanceof cn.seeyourface.ClassLoaderTest);

        ClassLoaderTest classLoaderTest = new ClassLoaderTest();
        System.out.println(classLoaderTest.getClass());
        System.out.println(classLoaderTest instanceof cn.seeyourface.ClassLoaderTest);
    }
}
```
...
```java
class cn.seeyourface.ClassLoaderTest
false
class cn.seeyourface.ClassLoaderTest
true
```

可以看出，代码中使用自定义类加载器（`myLoader`）加载的类和通过应用程序类加载器加载的类不是同一个类。综上，类加载器在`JVM`中的作用有：
...
- **`AppClassLoader`(应用程序类加载器)**：面向我们用户的加载器，负责加载当前应用类路径 `classpath` 下的所有 `jar` 包和类。
...
![类加载器层次结构](https://image.seeyourface.cn/2024/01/0c2e2eb7811f32d0b10a98c93f34e390.png)

除了 `BootstrapClassLoader` 是 JVM 自身的一部分之外，其他所有的类加载器都是在 JVM 外部实现的，并且全都继承自 `ClassLoader`抽象类。这样做的好处是用户可以自定义类加载器，以便让应用程序自己决定如何去获取所需的类。

每个 `ClassLoader` 可以通过`getParent()`获取其父 `ClassLoader`，如果获取到 `ClassLoader` 为`null`的话，那么该类是通过 `BootstrapClassLoader` 加载的。这是因为`BootstrapClassLoader` 由 C++ 实现，由于这个 C++ 实现的类加载器在 Java 中是没有与之对应的类的，所以拿到的结果是 null。
...
public class ClassLoaderTest {
    public static void main(String[] args) {
        ...
        while (needContinue){
            System.out.println(tab.toString() + classLoader);
            if(classLoader == null){
                needContinue = false;
            }else{
...
- 我们编写的 Java 类 `ClassLoaderTest` 的 `ClassLoader` 是`AppClassLoader`；
...
除了 `BootstrapClassLoader` 其他类加载器均由 Java 实现且全部继承自`java.lang.ClassLoader`。如果我们要自定义自己的类加载器，也需要继承 `ClassLoader`抽象类。
...
通过自定义类加载器我们可以满足自己的特殊需求。就比如说，我们可以对 Java 类的字节码（ `.class` 文件）进行加密，加载时再利用自定义的类加载器对其解密。
...
光内置的类加载器就有三种之多，当我们想要加载一个类的时候，具体是哪个类加载器加载呢？双亲委派模型会给你答案。
...
![tomcat-class-loader-parents-delegation-model](https://image.seeyourface.cn/2024/01/6c83c58cc5b97edca3ef2db2abe6ae7b.png)
...
[^1]: 深入分析 Java ClassLoader 原理：https://blog.csdn.net/xyang81/article/details/7292380
[^2]: Java 类加载器(ClassLoader)：http://gityuan.com/2016/01/24/java-classloader/
[^3]: Class ClassLoader - Oracle 官方文档：https://docs.oracle.com/javase/8/docs/api/java/lang/ClassLoader.html
[^4]:  https://javaguide.cn/java/jvm/classloader.html
